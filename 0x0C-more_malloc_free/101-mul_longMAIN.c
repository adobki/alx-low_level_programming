...
int main(int argc, char *argv[])
{
	...
	argv[1] += rmvLead(argv[1]), argv[2] += rmvLead(argv[2]);

	len[1] = strLen(argv[1]), len[2] = strLen(argv[2]);
	len[0] = len[1], len[3] = 1, len[4] = 2;
	if (len[2] > len[1])
		len[0] = len[2], len[3] = 2, len[4] = 1;

	len[5] = len[1] + len[2] - len[0], len[9] = len[1] + len[2];
